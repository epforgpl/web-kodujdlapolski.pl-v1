        ...
        <article class="Project-body-description row">
            ...
            <?php while (have_rows('linki')) : the_row(); ?>
                <a href="<?php the_sub_field('link'); ?>" target="_blank"
                   class="Project-body-description-link"><?php the_sub_field('title'); ?> <i>&rsaquo;</i></a>
            <?php endwhile; ?>
        </article>
            ...
            <?php if (have_rows('lookingfor')): ?>
                <?php while (have_rows('lookingfor')): the_row(); ?>
                    <div class="Team-row Team-row--add">
                        ...
                        <h2 class="Team-row-title"><a class="open_ajax"
                                                      href="<?php bloginfo('template_url'); ?>/framework/project-email.php?id=<?php echo $post->ID; ?>&pozycja=<?php echo urlencode(get_sub_field('pozycja')); ?>">Dołącz
                                jako <br><?php the_sub_field('pozycja'); ?></a></h2>
                    </div>
                <?php endwhile; ?>
            <?php endif;
            wp_reset_postdata(); ?>
